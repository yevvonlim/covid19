...
x = np.linspace(X_train[:,0].min(), X_train[:,0].max(), len(X_train[:,0]))
y = np.linspace(X_train[:,1].min(), X_train[:,1].max(), len(X_train[:,1]))
...
Z = np.exp(pipeline.score_samples(XX)) + e
Z = Z.reshape(X.shape)
...
m = Basemap(projection='merc', lat_0=37.35, lon_0=126.58, resolution = 'h',
               urcrnrlat=X_train[:,1].max(), llcrnrlat=X_train[:,1].min(), llcrnrlon=X_train[:,0].min(), urcrnrlon=X_train[:,0].max())
            
x_, y_ = m(X, Y) #위도, 경도를 플롯에 맞는 좌표로 변환해줌.


CS = plt.contourf(x_, y_, Z, cmap=plt.cm.Reds, levels=np.linspace(0, Z.max(),300))   #, 

CB = plt.colorbar(CS, shrink=0.8, extend='both')
plt.scatter(X_train[:, 0], X_train[:, 1], .8)

plt.title(date+' Danger Level of COVID-19 in South Korea')
# plt.axis('tight')

# --------------------test data-----------------------------------
oksu_station = [37.541706528, 127.017295229]  #(위도, 경도) = (y, x)
my_house = [35.82808984087305, 128.526564]
imae = [37.3955583072208, 127.12820494262064]
# ----------------------------------------------------------------



prob = numerical_2Dintegral(pipeline, oksu_station[1], oksu_station[0]) * 100
warning =("옥수역의 " + date +" 위험도는" + str(prob))


time = 24
prob = calc_critical_score(pipeline, imae[1], imae[0]) 
warning = "이매역의 " + date +" 위험도는" + str(prob)


print(warning)


plt.scatter(*m(imae[1], imae[0]), c='green')
plt.text(*m(imae[1]+0.01, imae[0]+0.01), "imae! :"+str(prob)[:4])
# plt.scatter(oksu_station[1], oksu_station[0], c='green')
# plt.text(oksu_station[1]+0.01, oksu_station[0]+0.01, "near ING house! :"+str(prob)[:4])
...
plt.savefig("/content/gdrive/MyDrive/공공데이터 경진대회(210529~)/kde_pdf_near777.png")
plt.show()
"""
